...
pytest -q                                            # 39 tests: the alignment gate + everything else
...
scripts/                verify_acceptance_tz, fetch_lm_dictionary, build_panel,
                        run_study, classify_sample, narrate_event, build_web_panel
docs/index.html         static results page (verdict + charts + full-universe narrator UI)
docs/panel_index.json   generated: lightweight company/event index for the search UI
vercel.json             Vercel config (serves docs/ statically, wires /api/narrate)
```
