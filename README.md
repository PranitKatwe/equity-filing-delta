...
**Signal #2, tested: meaning change in embedding space.** Motivated by Didisheim, Kelly, Pourmohammadi & Tian, ["The Inefficient Pricing of News"](https://www.nber.org/papers/w35093) (NBER w35093): represent text as an embedding and treat only the part not explained by firm characteristics as news. Each Item 1A is embedded locally (bge-large-en-v1.5 via fastembed, chunked and mean-pooled, GPU when available, zero API cost; [`embed.py`](src/eqd/delta/embed.py)); `emb_delta` is the cosine distance between consecutive years, and `emb_delta_resid` strips what sector and section size predict using an expanding fit on prior years only, so nothing leaks backward ([`residualize.py`](src/eqd/study/residualize.py), with an explicit no-lookahead test). Run through the same harness, **in-sample only** (the holdout was spent on the original signal), across all 4,705 events:

| Test (in-sample, 2,529 events) | `emb_delta` | `emb_delta_resid` |
|---|---|---|
| Placebo [-5,-1] (want insignificant) | p=0.34, clean | p=0.82, clean |
| CAR[0,+5] coef per 1 SD | -0.10% (p=0.56) | -0.10% (p=0.63) |
| CAR[0,+21] coef per 1 SD | +0.30% (p=0.19, wrong sign) | +0.40% (p=0.13, wrong sign) |
| Long-short spread, net of costs | -0.43% (p=0.47) | |

Same verdict as the mechanical count: nothing significant, sign-unstable across horizons, negative after costs. That is informative rather than disappointing: the paper's signal works on monthly news across thousands of names, and at one filing per company per year there are too few independent bets for a faint text effect to clear costs. Reproduce with [`build_embed_features.py`](scripts/build_embed_features.py) then [`run_study_embed.py`](scripts/run_study_embed.py).
...
**Grounded filing Q&A (RAG).** A second endpoint ([`api/ask.py`](api/ask.py)) answers plain-English questions about a company's most recent 10-K risk-factor changes, like *"what new risks did Boeing add this year?"*, strictly from the **actual added / removed / reworded sentences** the harness diffs out of that filing ([`api/_passages.py`](api/_passages.py), generated by [`scripts/build_passages.py`](scripts/build_passages.py)). Same discipline as the narrator: the model is grounded only in real, extracted filing text, cites the accession, says "not covered in the filing changes" when the answer isn't there, and never predicts or advises. The question is free text but only steers *what is summarized*: the model's entire context is that one company's real diff passages (question length-capped, rate-limited, cached).
...
scripts/                verify_acceptance_tz, fetch_lm_dictionary, build_panel, run_study,
                        classify_sample, classify_batch, build_embed_features, run_study_embed,
                        narrate_event, build_web_panel, build_passages
